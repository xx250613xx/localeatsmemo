...
export default function Header() {
    ...
    return (
        <header className="bg-gradient-to-r from-brand-dark via-brand-dark to-brand-light text-white p-4 relative z-10">
                ...
                <ul className="flex flex-col space-y-2 md:flex-row md:space-y-0 md:space-x-4 p-2 rounded">
                    {allpage.map((page) => {
                        return (
                            <li
                                key={page}
                                className="hover:underline bg-brand-dark rounded-md p-2 md:p-1 w-full md:w-auto"
                            >
                                <Link
                                    href={`/${page}`}
